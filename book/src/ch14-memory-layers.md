...
| L1 | 关键事实——最重要的记忆 | ~120 token（AAAK）/ ~500-800 token（原文） | 始终加载 | 最小可用上下文：团队、项目、核心偏好 |
| L2 | 房间回忆——按需检索 | ~200-500 token/次 | 话题出现时 | 当前话题的近期历史和相关上下文 |
...
L0 + L1 构成"唤醒成本"——大约 170 个 token（使用 AAAK 压缩时），或 600-900 个 token（使用原文时）。这意味着你的 AI 在每次新会话开始时，只需要消耗不到一千 token 就能"记住"你的整个世界。剩余 95% 以上的上下文窗口留给了当前对话。

按 README 中公布的数据，这个唤醒成本折算成年度费用大约是 **$0.70/年**——假设每天启动一次新会话，每次加载 170 token 的 AAAK 压缩文本。
...
**为什么是 ~120 token（AAAK）和 ~500-800 token（原文）？** 这个范围不是拍脑袋决定的。README 中的数据表明，MemPalace 的目标是让唤醒成本（L0 + L1）控制在 170 token 左右。在使用 AAAK 压缩（30 倍压缩率）时，L1 大约占 120 token——足以包含团队成员、当前项目、关键决策和核心偏好。不使用 AAAK 时，同样的信息量占 500-800 token，仍然在可控范围内。

这个 token 预算的确定方法是反向推导的：先确定"唤醒后，AI 应该能回答哪些问题"（你是谁、你的团队是谁、当前在做什么项目、最近做了什么重要决策），然后计算回答这些问题所需的最小信息量，再加上安全余量。120 个 AAAK token 就是这个最小信息量。
...
L0 和 L1 始终在场，它们构成了 AI 的"常驻意识"。L3 是深度搜索，需要明确的查询。L2 的角色在两者之间：当对话中自然出现某个话题时——比如用户提到了某个项目名称，或者讨论转向了某个技术领域——L2 自动加载与该话题相关的记忆片段。
...
注意这里用的是 `col.get()` 而非 `col.query()`。`get()` 是 ChromaDB 的元数据过滤方法，不涉及向量相似度计算——它只是按条件返回匹配的文档。这意味着 L2 的检索是确定性的、零语义成本的。当用户说"我们来看看 Driftwood 项目"时，L2 不需要理解这句话的"含义"，它只需要找到所有 `wing=driftwood` 的记忆。

**为什么是 200-500 token？** 这个范围对应的是一个话题下最近的 5-10 条记忆片段。每条片段被截断到 300 字符以内（`layers.py:226-228`），加上元数据标签，总量控制在一到两段话的长度。这个量足以让 AI "想起"当前话题的近期脉络，但不会挤占对话本身的上下文空间。

L2 的存在解决了一个微妙但重要的用户体验问题：如果没有 L2，AI 要么对所有话题都只有 L1 级别的浅层了解（不够用），要么每次话题转换都需要用户手动触发搜索（太麻烦）。L2 让"自然地切换话题"成为可能——当你提到一个项目，与之相关的记忆自动浮现，就像人类在对话中自然地回忆相关经历一样。
...
前面三层都在做"预加载"——基于规则和结构，在对话开始前或话题切换时自动注入相关信息。L3 不同，它是按需触发的全量搜索，用于回答需要在整个记忆库中检索的问题。
...
- `recall()`：话题转换时调用。当对话涉及特定项目或领域时，加载相关记忆。
...
**为什么不是两层（始终加载 + 搜索）？** 因为"始终加载"和"搜索"之间存在一个重要的灰色地带。想象一下只有 L0+L1 和 L3 的系统：你的 AI 知道你的名字和当前项目（L1），如果被问到具体问题可以搜索（L3），但当对话自然转向一个相关话题时，它不会自动加载上下文。用户要么需要不断手动触发搜索，要么忍受 AI 对当前话题的浅薄理解。L2 填补了这个空隙——它是"话题感知的预加载"，让对话流更自然。
...
**为什么不是更多层？** 因为每增加一个层级，就增加了一个"何时加载"的决策点。四层已经覆盖了所有关键的时间语义：始终（L0、L1）、按话题（L2）、按查询（L3）。你很难定义出第五种有意义的加载时机。如果试图把 L2 拆分成"近期话题"和"远期话题"，或者把 L3 拆分成"浅度搜索"和"深度搜索"，带来的复杂性很可能超过收益。
...
核心数字是：MemPalace 唤醒仅需 ~170 token，年度成本约 $0.70；即使加上每日 5 次深度搜索也不过 ~$10/年——与 LLM 摘要方案的 ~$507/年形成鲜明对比（完整的方案成本对比详见第 23 章）。

$0.70/年的唤醒成本——这个数字的意义不在于它有多便宜，而在于它揭示了一个设计原则：**最好的成本优化不是让每次调用更便宜，而是让大多数调用不发生。**

L0 + L1 的 170 个 token 不是被"压缩"出来的——它们是被"选择"出来的。通过四层分层，95% 以上的记忆永远不需要加载到任何对话的上下文中。它们安静地待在 ChromaDB 里，只有在被明确需要时才通过 L2 或 L3 出场。

这就是分层的价值：不是让你存更少的东西，而是让你在正确的时刻加载正确的量。六个月的完整记忆仍然在那里，一条都没有丢。但你的 AI 只需要 170 个 token 就能"醒来"并表现得好像它记得一切。
