...
详见第19章记录的 5 个认知组中的 19 个 MCP 工具（基线：v3.0.0）。这个设计在学理上是自洽的——每个组对应 AI 与记忆交互时扮演的一个角色。v3.3.0 把工具数扩展到 29 个（新增的 10 个是 Tunnel CRUD、Drawer 读写 CRUD、运维工具），但五组认知角色的骨架不变——详见第19章章末的版本演化说明。
...
精简并不是在判定 MemPalace 的工具面是错误的。它反映的是不同的实现成熟度阶段，以及关于自文档化的设计选择。

v3.0.0 的 19 个工具中有 8 个属于 Knowledge Graph（5个）和 Navigation（3个）组。它们依赖于完整填充的知识图谱和可工作的图遍历引擎——附录 D 指出这些子系统"被叙述得多于被实际使用"。为尚不可靠的子系统提供工具，会误导 agent 去调用它们并得到糟糕的结果。v3.3.0 的 29 个工具只是在这之上进一步填平了 CRUD 缺口（drawer 的 get/list/update、tunnel 的显式 CRUD）和运维面（`reconnect`、`hook_settings`、`memories_filed_away`）——原来的 8 个弱子系统依赖问题并未被解决。

5 个工具的接口还能提供更丰富的逐工具文档。mempal 中每个工具都携带详细的字段级文档——`SearchRequest` 上的 `wing` 字段（`crates/mempal-mcp/src/tools.rs:11-16`）详细解释了何时应该省略它，并警告盲猜会静默返回零结果。如果有 29 个工具，这种粒度的字段文档会让工具列表响应不堪重负。5 个工具的情况下，每个都可以做到充分的自文档化。
