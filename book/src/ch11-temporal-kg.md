...
**`source_closet`** —— 指向记忆宫殿中的 closet。这是知识图谱和宫殿结构之间的桥梁：每条三元组都可以追溯到它来自哪个 closet，从而追溯到原始的逐字记忆。当你质疑"这条事实的依据是什么"时，系统可以把你带回当初那段对话的原文。

**`source_file`** —— 原始文件路径。比 `source_closet` 更底层的溯源信息。
