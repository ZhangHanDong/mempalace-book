...
本章关于 Hall 是"解释设计的中间层，而非独立查询参数"的判断，在 v3.3.0 被部分推翻。

### Hall 从"叙述"走向"实现"

v3.3.0 在 `config.py:74-112` 定义了 `DEFAULT_HALL_KEYWORDS`——将 7 个 Hall 与关键词列表绑定：
...
drawer 在摄入时经由关键词匹配被路由到对应的 Hall（PR `#835`）——`DEFAULT_HALL_KEYWORDS` 与本章提到的 `DEFAULT_TOPIC_WINGS`（`config.py:64-72`）**共用同一组 7 个名字但角色不同**：`DEFAULT_TOPIC_WINGS` 是可选的"主题型 wing"名称列表；`DEFAULT_HALL_KEYWORDS` 是 wing 内部用于把 drawer 路由到某个 Hall 的关键词表。v3.3.0 的 README 审计（PR `#835`，42 个回归保护测试）确认 Hall 检测作为可验证能力存在，而不只是叙事。

### 本章判断的修订

- **v3.0.0 版本**："Hall 作为分类概念存在，但不是默认的路由目标。"
- **v3.3.0 版本**：Hall 是默认的路由目标——每个 drawer 在摄入时都会被分配一个 Hall。但用户仍不能通过 MCP `search` 工具的参数直接过滤 Hall（这一层暴露还在设计中）。
- 用户仍可在 `~/.mempalace/config.json` 的 `hall_keywords` 字段覆盖默认关键词，把 Hall 定义适配到自己的语言习惯。

### 对第 7 章 "+Hall: 84.8%" 基准的影响

本章结尾引用的"+Hall: 84.8%"提升是 v3.0.0 的历史基准。v3.3.0 的 Hall 检测才真正让这个数据有持续复现的可能——v3.0.0 时 Hall 更多是元数据标签而非路由主驱动，+12% 的增益在 v3.0.0 的默认 pipeline 上很难复跑。v3.3.0 的基准应单独重跑确认。
