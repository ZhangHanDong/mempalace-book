...
详见第19章对 MemPalace v3.0.0 时 19 个 MCP 工具的分析（v3.3.0 扩展到 29 个，详见第19章版本演化说明）。工具描述告诉 agent 每个工具*做什么*，但没有告诉 agent *什么时候*该用记忆而非直接 grep 文件、*怎样*在过滤前先发现合法的 wing 名称、以及*为什么*引用很重要。这些行为模式写在 README 和项目指南里——MCP 连接的 agent 永远看不到的地方。
...
详见第19章对 MemPalace 按 5 种认知角色组织的 19 个工具的分析（基线 v3.0.0；v3.3.0 已增至 29 个，见该章版本演化说明）。mempal 有 7 个工具。本节解释为什么精简是可行的——以及它依赖什么条件。
...
综合协议、工具和检索——以下是典型 agent session 的完整生命周期：
...
这个生命周期完全是自引导的。agent 在连接时接收协议，通过 `mempal_status` 发现 wing，使用混合检索进行搜索，以重要性排序保存决策，并可选地在日记中记录行为观察。无需任何外部配置。
