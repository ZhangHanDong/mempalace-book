...
每个脚本接受数据集路径和模式参数，输出标准格式的结果文件。`benchmarks/BENCHMARKS.md` 记录了从 96.6% 基线到 100% 满分的完整改进过程——不是作为营销材料，而是作为一份技术实验记录，详细记录了每一步改进的动机、方法和结果。
...
从 99.4% 到 100% 的最后一步，是通过分析两种独立架构（hybrid v4 和 palace mode）共同失败的三个问题，针对每个问题设计了专门的修复：引号短语提取、人名加权和记忆/怀旧模式匹配。这种"分析失败 -> 设计修复 -> 验证效果"的循环是工程上可靠的改进方法——不是调参，而是理解失败原因。
