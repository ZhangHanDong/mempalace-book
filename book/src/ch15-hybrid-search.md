...
本章的核心分析（混合路径设计、96.6% 到 100% 的工程故事、两条独立路径收敛）在 v3.3.0 仍然成立。但有两项底层修复直接影响本章数据能否复现——如果你要重跑基准，必须在 v3.3.0 下跑。

### Issue #807：ChromaDB 距离度量修复

v3.0.0 在创建 collection 时没有显式声明距离度量，ChromaDB 默认落到 **L2（欧氏距离）**——而全书（包括本章）的分析都假设使用**余弦相似度**。两者在长度归一化的嵌入向量上行为相近，但在未归一化向量上可以给出截然不同的近邻排序。这意味着 v3.0.0 的 96.6% 和 v4 的 100% 都是在 L2 下跑出来的——本章引用的 `add_drawer` 0.9 余弦阈值去重、`search` 的 cosine 距离过滤等设计，在 v3.0.0 实际运行时可能**跑的是 L2**。

`#807` 在所有 collection 创建点补齐了 `{"hnsw:space": "cosine"}` 元数据。v3.3.0 下的基准才真正落地为余弦。**如果你要复现 96.6% → 100% 的数字，请在 v3.3.0 下重跑。**

### BM25 + Closet boost

v3.3.0 引入 Closet 层作为 drawer 之上的紧凑索引（PR `#788`），BM25 在 Closet 上跑（`#795`、`#829`），匹配结果对下层 drawer 做 boost 而非 gate——也就是说 BM25 不会漏掉向量能找到但 BM25 找不到的 drawer。这比本章描述的 hybrid v1→v4 更接近工业级 RAG 的做法（典型是 BM25 召回 + 向量 rerank，或两者融合）。

本章的 hybrid 路径是**问题分类 + 路径选择**的混合；v3.3.0 的 BM25 + 向量是**召回层面的混合**。两种"混合"解决不同的问题，可以叠加——未在本章基准中验证过叠加收益，值得作为后续实验题。

### diff 快照

- v3.0.0：向量检索 + 可选 hybrid 路径 + L2 默认距离（设计意图是余弦）
- v3.3.0：向量检索 + BM25 closet boost + 强制 cosine + 可选 hybrid 路径

本章所有百分比数字未在 v3.3.0 下重跑。读者如需严格引用，请以 v3.0.0 为基线理解。
