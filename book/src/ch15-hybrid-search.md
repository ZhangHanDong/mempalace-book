...
README 中引用的 "$0.70/年" 更具体地指的是唤醒成本——L0 + L1 每天加载一次，每次 ~170 token。这与重排序是两个不同的成本项，但量级是一致的：都在"年度一美元"的范围内。
...
100% R@5 是在 LongMemEval 的 500 个问题上测得的。这 500 个问题覆盖了六种类型，由学术团队设计，是目前 AI 记忆系统最标准的评估基准。这个分数是可重复的、经过验证的、有完整的复现脚本的。
...
96.6% 和 100% 是同一个系统的两个侧面。前者是无条件保证——不依赖任何外部服务，在任何环境下都能工作；后者是在标准基准上的竞争成绩——但需要 API 密钥和网络连接。用户需要根据自己的场景选择使用哪种模式。
