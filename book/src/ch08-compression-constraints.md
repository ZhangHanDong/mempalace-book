...
MemPalace 的设计目标是让 L1 层控制在约 120 个 token 以内。这意味着原本可能需要数千 token 描述的团队结构、项目状态、关键决策和个人偏好，必须被压缩到约 30 倍以下。

为什么是 30 倍而不是 10 倍或 100 倍？这个数字并非凭空而来。10 倍压缩可以通过简单的去冗余实现（去掉冠词、介词、重复描述），但这只能把 1000 token 压缩到 100 token——对于一个完整的团队描述来说仍然不够紧凑。100 倍压缩则几乎必然意味着信息损失，因为英语的信息熵决定了你无法用 10 个 token 无损表达 1000 token 的内容。

30 倍是这个工程权衡中的甜蜜点：足够激进以实现真正的 token 经济，又足够保守以保证信息完整性。

### 约束二：零信息损失

这是与所有"摘要式"记忆系统的根本分界线。
...
MemPalace 的立场是：压缩后的表示必须包含原始文本中的所有语义信息。读到压缩结果的模型，应该能够回答关于原始内容的任何问题，包括那些在压缩时看起来不重要但事后被证明关键的细节。

"零信息损失"需要一个操作性的定义。在 MemPalace 的语境中，它的意思不是"压缩后的文本和原始文本逐字相同"（那就不是压缩了），而是"对于原始文本中的每一个事实断言，压缩后的文本中都存在对应的编码，使得一个合格的语言模型能够正确还原该断言"。

这个定义把"信息"限定在事实断言的层面——谁、做了什么、什么时候、为什么、结果如何——而不包括文体风格、修辞手法或措辞偏好。这是一个有意识的边界划定：MemPalace 存储的是记忆，不是文学作品。
...
**必须保留所有事实断言**——因为约束二要求零信息损失，每个实体、关系、属性和事件都必须在压缩后有对应表示。

**必须极度紧凑**——因为约束一要求 30 倍压缩比，每个 token 必须携带远高于自然英语的信息密度。
...
这些冗余加在一起，为约束一要求的 30 倍压缩提供了充足的空间。但关键在于：去除冗余不等于丢失信息。冗余是信息的额外包装，去掉包装不改变内容物，前提是接收方有能力从裸露的内容物中重建完整的理解。
...
2. **约束定义**：30 倍压缩、零损失、模型通用、无需解码器。四个约束缺一不可。
