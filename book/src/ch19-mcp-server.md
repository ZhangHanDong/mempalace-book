...
本章基于 v3.0.0 的 19 工具快照分析。v3.3.0 的 `TOOLS` 字典（`mcp_server.py:1111`）已增长到 29 个工具——新增的 10 个分为三类：

**Tunnel 显式管理（4 个，CRUD + traversal）**：`create_tunnel` / `list_tunnels` / `delete_tunnel` / `follow_tunnels`（`mcp_server.py:1257-1315`）——前三是 CRUD，`follow_tunnels` 是只读遍历。v3.0.0 的 `find_tunnels` 是"只读发现"——系统自动从同名 room 推断隧道。v3.3.0 让用户可以**显式声明**跨 wing 链接（比如手动连 `wing_myproject` 的 `architecture` room 和 `wing_research` 的 `papers` room）。这不改变导航组的认知角色，只是把隧道从"涌现结构"扩展为"涌现 + 声明"。

**Drawer 读写 CRUD（3 个）**：`get_drawer` / `list_drawers` / `update_drawer`（`mcp_server.py:1394-1449`）。v3.0.0 的读取组对 drawer 粒度只能通过 `search` 间接到达，写入组只有 `add_drawer` / `delete_drawer`。v3.3.0 填平了 CRUD 的空缺——按 ID 取单条、分页列表、原地更新。本章"读取 7 / 写入 2"的不对称判断在 v3.3.0 仍然成立（读取 9 / 写入 3），因为新增的读取入口都是**粒度变化**而非新认知模式。

**运维工具（3 个）**：`hook_settings` / `memories_filed_away` / `reconnect`（`mcp_server.py:1490-1527`）。这三个严格说不属于原五组认知角色——它们是暴露给 AI 的**系统自检面**：hook 静默开关、checkpoint 状态查询、强制重连。`reconnect` 的 description 直言其用途是"外部脚本或 CLI 直接修改 palace 后可能使内存中的 HNSW 索引失效"。本章论证的"五组认知角色"是应用层模型，运维工具属于另一个维度，不破坏原分组——但它确实说明一件事：**当 MCP 成为 AI 和系统之间唯一的交互通道时，系统自身的运维也必须暴露为工具**。

### 本章原判断的修订

- **"19 是最小完备集"** → 调整为"19 是**认知角色层面**的最小完备集"。v3.3.0 的增量主要在 CRUD 粒度和运维维度，不在认知角色维度。
- **"读取多于写入的 7:2 不对称"** → 在 v3.3.0 中是 9:3，不对称性不变。
- **`mempalace_status` 三重载荷** → 不变，仍是 AI 进入宫殿的唯一教学入口。
- **Part 10 分析的 mempal（Rust 重铸版）把工具精简到 5-7 个**（第 27 章表述为 5，第 28 章表述为 7）→ 对比基数在 v3.3.0 下从 19→5-7 变成 29→5-7，精简幅度反而更显著，Part 10 的精简论点更强而非更弱。

### 修复层面的改动（不改变工具数）

- `#807` 把所有 collection 的 `hnsw:space=cosine` 元数据补齐——v3.0.0 在 ChromaDB 默认 L2 距离下相似度评分失真，本章提到的"余弦相似度 0.9 去重阈值"在 v3.0.0 实际跑的是 L2，v3.3.0 才真正落地为余弦。
- `add_drawer` 的 drawer ID 生成（v3.0.0 是内容前 100 字符 + 时间戳 MD5 前 16 位）在 v3.3.0 改为 full content hash（`#716`）——稳定 re-mine，避免重复摄入产生新 ID。
