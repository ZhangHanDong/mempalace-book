...
2026 年初，这条推文在技术社区引发了一场不大不小的震动。震动的原因不在于又一个 AI 记忆产品问世——市场上从来不缺这样的产品——而在于两件不寻常的事实：第一，它在 LongMemEval 基准测试中取得了有史以来的第一个满分（500/500，R@5 = 100%）；第二，它的两位创始人的组合实在令人意外。
...
**这不是一本教程，而是一本设计分析。** 你不会在这里找到"第一步安装，第二步配置"的操作指南——MemPalace 的 README 和文档已经做得足够好了。这本书关心的是更深层的问题：为什么古希腊的记忆术原理能在大语言模型时代重新生效？为什么一个零 API 调用的本地系统能在检索精度上达到 96.6%，加上一次轻量级重排序就能冲到 100%？为什么一种面向 AI 的压缩方言能实现 30 倍压缩而零信息损失？为什么放弃"让 AI 决定什么值得记住"反而带来了更好的结果？
...
**AAAK 压缩方言。** 这是一种专为 AI 智能体设计的无损速记语言。它不是给人读的——它是给你的 AI 读的，而且读得很快。30 倍压缩，零信息损失。你的 AI 在大约 120 个 token 内加载数月的上下文。因为 AAAK 本质上只是结构化文本加通用语法，它可以与任何能读取文本的模型协作——Claude、GPT、Gemini、Llama、Mistral——不需要解码器，不需要微调，不需要云端 API。
...
- **第三部分（第 8-10 章）：AAAK 压缩语言** ——理解这种面向 AI 的无损压缩方言是如何设计的，以及为什么它能在任何模型上工作。这可能是 MemPalace 最具独创性的贡献。
