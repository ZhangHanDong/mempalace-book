...
**这不是一本教程，而是一本设计分析。** 你不会在这里找到"第一步安装，第二步配置"的操作指南——MemPalace 的 README 和文档已经做得足够好了。这本书关心的是更深层的问题：为什么古希腊的记忆术原理能在大语言模型时代重新生效？为什么一个零 API 调用的本地系统能在检索精度上达到 96.6%，加上一次轻量级重排序能在完整 benchmark 上冲到 100%，而同一套文档又会诚实地给出 98.4% 的 held-out 数字？为什么一种面向 AI 的压缩方言能实现 30 倍压缩而零信息损失？为什么放弃"让 AI 决定什么值得记住"反而带来了更好的结果？
...
**AAAK 压缩方言。** 这是一种专为 AI 智能体设计的无损速记语言。它不是给人读的——它是给你的 AI 读的，而且读得很快。30 倍压缩，零信息损失。在 README 展示的目标路径里，AI 可以在大约 120 个 token 的 AAAK 上下文中加载数月的内容；而当前仓库默认的 `wake-up` 仍然主要输出约 600-900 token 的 L0 + L1 文本。因为 AAAK 本质上只是结构化文本加通用语法，它可以与任何能读取文本的模型协作——Claude、GPT、Gemini、Llama、Mistral——不需要解码器，不需要微调，不需要云端 API。
