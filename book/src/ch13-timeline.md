...
时间线提供了概貌；如果知识图谱条目在写入时带上了 `source_closet` 链接，它就还能继续追溯到原始的对话记录或文档，宫殿结构的语义搜索则提供完整的上下文。三个系统协同工作，形成了从"全景"到"特写"的信息检索链条。
...
**宫殿结构**提供上下文和原始记忆。更准确地说，当时间线中的事实带有 `source_closet` 或 `source_file` 这类溯源字段时，它就可以回指到宫殿中的相应位置，再进一步连接到原始逐字内容。当前 schema 支持这条链路，但是否存在，取决于写入知识图谱时是否提供了这些字段。
