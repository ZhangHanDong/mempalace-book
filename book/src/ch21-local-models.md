...
> **定位**：本章阐释 MemPalace 如何在完全离线的环境中运行——从 ChromaDB 到本地模型到 AAAK 压缩——以及为什么整个栈的设计从第一天起就把"无需网络"作为硬约束而非可选特性。
...
MemPalace 的设计方向完全相反：**本地是唯一模式，云端是可选增强。** ChromaDB 是嵌入式向量数据库，数据存储在本地文件系统。知识图谱用 SQLite，同样是本地文件。AAAK 压缩是纯字符串操作，不依赖任何外部服务。MCP 服务器走 stdio 通道，不涉及网络。整个栈从第一行代码开始就可以在一台断网的笔记本上运行。
...
输出大约 170 token。这个数字不是巧合。它被设计为可以塞进任何模型的系统提示而不占用显著的上下文空间——即使是 4K 上下文的小模型，170 token 也只占 4%。

为什么用 wake-up 这个名字而不是 "load-context" 或 "get-summary"？因为这个操作的语义不是"获取数据"，而是"唤醒一个有记忆的代理"。当本地模型加载了这 170 token 后，它从一个对用户一无所知的通用模型，变成了一个知道用户是谁、在做什么项目、关心什么事情的专属助手。这是身份的注入，不是数据的传输。
...
把所有组件放在一起看，一个完全离线的 MemPalace 栈长什么样：

**存储层：ChromaDB（嵌入式）+ SQLite。** ChromaDB 在本地文件系统中存储向量嵌入和文档，默认位置 `~/.mempalace/palace`。它自带嵌入模型——ChromaDB 使用 all-MiniLM-L6-v2（一个 80MB 的 sentence transformer）作为默认嵌入器，在首次使用时自动下载，之后完全离线运行。知识图谱用 SQLite，存储在 `~/.mempalace/knowledge_graph.sqlite3`。两个数据库加起来对磁盘的要求微乎其微——一个有 22,000 条记忆的宫殿，全部数据加索引大约 200-300MB。

**压缩层：AAAK 方言。** 纯规则驱动的文本压缩，不依赖任何模型。实体名替换为三字母代码，结构化为管道分隔格式，情绪标记为星号标记。30 倍压缩率意味着 3000 token 的自然语言记忆可以压缩到 100 token。这对小上下文窗口的本地模型尤为重要——一个 4K 上下文的模型，扣除系统提示和用户输入后可能只剩 2K token 给记忆。AAAK 让这 2K token 装得下原本需要 60K token 的内容。
...
这个选择的更深层意义在于：它不要求本地模型有任何"特殊能力"。不需要 function calling 支持，不需要 tool use 训练，不需要 JSON mode。一个只经过基础文本生成训练的 7B 模型，粘贴 170 token 的 wake-up 文本到系统提示里，就能访问用户的记忆。准入门槛是零。
...
**wake-up 路径适合交互式使用。** 用户坐在终端前，开启一个新的对话，先运行 `mempalace wake-up`，把输出粘贴到模型的上下文中，然后开始对话。整个过程需要 10 秒，额外 token 消耗 170。适合日常的问答、头脑风暴、代码审查。它的优势是零集成成本——不需要写代码，不需要改配置，不需要搭管线。
