...
没有 API key。没有云端服务。安装完成后不需要互联网连接。
...
MemPalace 选择了一个更激进的立场：**安装之后，拔掉网线，一切照常工作。**
...
这个约束的工程含义是深远的。它意味着 MemPalace 不能使用任何需要网络调用的功能——即使那些功能可以显著提升性能。例如，用一个云端的大型嵌入模型（如 OpenAI 的 text-embedding-3-large）来替代本地的 all-MiniLM-L6-v2，几乎肯定可以提升检索精度。但这样做就引入了对外部服务的依赖，打破了零依赖约束。

MemPalace 的选择是：用本地嵌入模型达到 96.6% 的精度，而不是用云端模型追求更高的分数。96.6% 在零 API 调用的条件下已经是有史以来的最高分。这个分数不是妥协的结果——它是在严格约束下取得的成就。

Haiku 重排序是一个有意思的设计点。它是 MemPalace 中唯一可选的网络功能——使用 Claude Haiku 对本地检索结果进行重排序，可以把精度从 96.6% 提升到 100%。但关键词是"可选"。不启用它，系统完全正常工作。启用它，你获得的是锦上添花而非雪中送炭。这个设计精确地表达了 MemPalace 对网络依赖的态度：可以有，但不能必须有。
...
**场景一：安全审计。** 一家金融科技公司的安全团队需要审计所有处理客户数据的系统。对于 SaaS 记忆产品，审计意味着审查第三方的安全认证、数据处理协议、子处理器列表和数据驻留政策。对于 MemPalace，审计意味着：阅读源代码，确认没有网络调用，结束。一个下午的代码审查替代了数周的合规文档审查。
...
MemPalace 用一个简洁的技术栈实现了这个姿态：Python + ChromaDB + SQLite + AAAK。没有服务器，没有 API，没有订阅，没有你无法审计的代码。你的记忆在你的机器上，处理记忆的代码在你的 GitHub fork 里，压缩记忆的 AAAK 方言是公开的规范。
